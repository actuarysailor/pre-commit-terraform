...
# Check if we're being called as a pre-commit hook directly
# This happens when using language: docker_image
if [[ "$1" == "--hook-config=--hook-id="* ]]; then
  # Extract hook ID from the argument
  HOOK_ID="${1#*=}"
  shift
  
  # Call the specific hook script directly
  exec "/usr/bin/hooks/${HOOK_ID}.sh" "$@"
fi

# Original entrypoint logic for running full pre-commit
